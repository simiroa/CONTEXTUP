...
class MarigoldGUI(BaseWindow):
    ...
    def _run_model_check(self):
        try:
            ...
            self.main_frame.after(0, lambda: self.lbl_status.configure(text="Verifying Models (This may take time)..."))
            
            success, output = run_ai_script("check_marigold_deps.py")
            
            if success:
                self.main_frame.after(0, lambda: messagebox.showinfo("Models", t("marigold_gui.models_ready")))
                self.main_frame.after(0, lambda: self.lbl_status.configure(text=t("marigold_gui.models_ready"), text_color="gray"))
            else:
                self.main_frame.after(0, lambda: messagebox.showerror("Error", f"{t('marigold_gui.model_check_failed')}:\n{output}"))
                self.main_frame.after(0, lambda: self.lbl_status.configure(text=t("marigold_gui.model_check_failed"), text_color="red"))
                 
        except Exception as e:
            print(e)
            self.main_frame.after(0, lambda: self.lbl_status.configure(text="Error checking models", text_color="red"))
        ...
        if found_any:
            ...
            if messagebox.askyesno(t("common.success"), f"{t('marigold_gui.maps_generated')}\n\n{t('marigold_gui.open_folder_prompt')}"):
                try:
                    ...
                    messagebox.showerror(t("common.error"), f"Could not open folder:\n{e}")
