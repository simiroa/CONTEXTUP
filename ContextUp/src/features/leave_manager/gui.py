...
class LeaveManagerGUI(ctk.CTk):
    ...
    def _build_add_tab(self):
        ...
        # Report Button (✨)
        self.btn_report = ctk.CTkButton(self.tab_add, text="✨ Export Special Report (HTML)", 
    ...
    def _render_history_list(self):
        ...
        if not filtered_history:
            ctk.CTkLabel(self.frame_history_content, text="No items found.").pack(pady=10)
            return
    ...
    def _batch_delete(self):
        ...
        if not items: return
        if tk.messagebox.askyesno("Delete", f"Delete {len(items)} selected items?"):
            for item in items:
    ...
    def _on_date_right_click(self, date_str):
        ...
        confirm = tk.messagebox.askyesno(
            t("common.delete"),
            f"{date_str} {t('leave_manager_gui.history')} {t('common.delete')}?"
        )
