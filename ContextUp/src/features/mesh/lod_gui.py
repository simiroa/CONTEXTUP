...
class AutoLODGUI(BaseWindow):
    ...
    def create_widgets(self):
        ...
        # Header
        self.add_header(f"Generate LODs for {len(self.files)} Files", font_size=16)
        ...
        # LOD Count
        ctk.CTkLabel(content, text="Number of LODs:", font=ctk.CTkFont(weight="bold")).grid(row=0, column=0, padx=10, pady=15, sticky="e")
        self.var_lod_count = ctk.IntVar(value=3)
        ctk.CTkComboBox(content, variable=self.var_lod_count, values=["2", "3", "4", "5"], width=70).grid(row=0, column=1, padx=10, sticky="w")
        
        # Reduction Ratio
        ctk.CTkLabel(content, text="Reduction per Step:", font=ctk.CTkFont(weight="bold")).grid(row=1, column=0, padx=10, pady=15, sticky="e")
