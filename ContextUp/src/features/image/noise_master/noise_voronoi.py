"""
Noise Master - Voronoi Noise (CPU).
"""

import numpy as np

from .noise_hash import hash_int2_to_float2, hash_int2_to_float3
from .noise_math import lerp as _lerp, smoothstep01 as _smoothstep01
...
def _voronoi_distance(dx, dy, metric, exponent):
    ...
    if metric == "minkowski":
        return (np.abs(dx) ** exponent + np.abs(dy) ** exponent) ** (1.0 / exponent)
...
def _voronoi_distance_bound(dx, dy, metric, exponent):
    ...
    if metric == "chebyshev":
        return np.maximum(np.abs(dx), np.abs(dy))
    if metric == "minkowski":
        return np.abs(dx) ** exponent + np.abs(dy) ** exponent
    return dx * dx + dy * dy


def _voronoi_f1(u, v, randomness, metric, exponent, seed):
    cell_x = np.floor(u).astype(np.int32)
    cell_y = np.floor(v).astype(np.int32)
    local_x = u - cell_x
    local_y = v - cell_y

    min_bound = np.full(u.shape, np.inf, dtype=np.float32)
    target_x = np.zeros_like(u, dtype=np.float32)
    target_y = np.zeros_like(v, dtype=np.float32)
    ...
    for oy in range(-1, 2):
        for ox in range(-1, 2):
            ...
            min_bound = np.where(mask, bound, min_bound)
            target_x = np.where(mask, point_x, target_x)
            target_y = np.where(mask, point_y, target_y)
...
def _voronoi_f2(u, v, randomness, metric, exponent, seed):
    ...
    d1 = np.full(u.shape, np.inf, dtype=np.float32)
    d2 = np.full(u.shape, np.inf, dtype=np.float32)
    ...
    for oy in range(-1, 2):
        for ox in range(-1, 2):
            ...
            bound = _voronoi_distance_bound(dx, dy, metric, exponent)
            mask1 = bound < d1
            d2 = np.where(mask1, d1, d2)
            d1 = np.where(mask1, bound, d1)
            mask2 = (~mask1) & (bound < d2)
            d2 = np.where(mask2, bound, d2)
...
def _voronoi_smooth_f1(u, v, randomness, metric, exponent, smoothness, seed):
    ...
    smooth_dist = np.zeros_like(u, dtype=np.float32)
    h = np.full(u.shape, -1.0, dtype=np.float32)
    ...
    for oy in range(-2, 3):
        for ox in range(-2, 3):
            ...
            h = np.where(h < 0.0, 1.0, _smoothstep01(0.5 + 0.5 * (smooth_dist - dist) / smoothness))
...
def _voronoi_f1_output(u, v, randomness, metric, exponent, seed, calc_color):
    ...
    target_x = np.zeros_like(u, dtype=np.float32)
    target_y = np.zeros_like(v, dtype=np.float32)
    target_cell_x = np.zeros_like(cell_x)
    target_cell_y = np.zeros_like(cell_y)
    ...
    for oy in range(-1, 2):
        for ox in range(-1, 2):
            ...
            target_x = np.where(mask, point_x, target_x)
            target_y = np.where(mask, point_y, target_y)
            target_cell_x = np.where(mask, cx, target_cell_x)
            target_cell_y = np.where(mask, cy, target_cell_y)
    ...
    if calc_color:
        ...
        cr = np.zeros_like(u, dtype=np.float32)
        cg = np.zeros_like(u, dtype=np.float32)
        cb = np.zeros_like(u, dtype=np.float32)
...
def _voronoi_f2_output(u, v, randomness, metric, exponent, seed, calc_color):
    ...
    d1 = np.full(u.shape, np.inf, dtype=np.float32)
    d2 = np.full(u.shape, np.inf, dtype=np.float32)
    pos1_x = np.zeros_like(u, dtype=np.float32)
    pos1_y = np.zeros_like(v, dtype=np.float32)
    pos2_x = np.zeros_like(u, dtype=np.float32)
    pos2_y = np.zeros_like(v, dtype=np.float32)
    cell1_x = np.zeros_like(cell_x)
    cell1_y = np.zeros_like(cell_y)
    cell2_x = np.zeros_like(cell_x)
    cell2_y = np.zeros_like(cell_y)
    ...
    for oy in range(-1, 2):
        for ox in range(-1, 2):
            ...
            old_cell1_y = cell1_y
            d1 = np.where(mask1, bound, d1)
            pos1_x = np.where(mask1, point_x, pos1_x)
            pos1_y = np.where(mask1, point_y, pos1_y)
            cell1_x = np.where(mask1, cx, cell1_x)
            cell1_y = np.where(mask1, cy, cell1_y)
            d2 = np.where(mask1, old_d1, d2)
            pos2_x = np.where(mask1, old_pos1_x, pos2_x)
            pos2_y = np.where(mask1, old_pos1_y, pos2_y)
            cell2_x = np.where(mask1, old_cell1_x, cell2_x)
            cell2_y = np.where(mask1, old_cell1_y, cell2_y)
            mask2 = (~mask1) & (bound < d2)
            d2 = np.where(mask2, bound, d2)
            pos2_x = np.where(mask2, point_x, pos2_x)
            pos2_y = np.where(mask2, point_y, pos2_y)
            cell2_x = np.where(mask2, cx, cell2_x)
            cell2_y = np.where(mask2, cy, cell2_y)
...
def _voronoi_smooth_f1_output(u, v, randomness, metric, exponent, smoothness, seed, calc_color):
    ...
    smooth_dist = np.zeros_like(u, dtype=np.float32)
    smooth_pos_x = np.zeros_like(u, dtype=np.float32)
    smooth_pos_y = np.zeros_like(v, dtype=np.float32)
    smooth_r = np.zeros_like(u, dtype=np.float32)
    smooth_g = np.zeros_like(u, dtype=np.float32)
    smooth_b = np.zeros_like(u, dtype=np.float32)
    h = np.full(u.shape, -1.0, dtype=np.float32)
...
def _voronoi_distance_to_edge(u, v, randomness, seed):
    ...
    min_dist = np.full(u.shape, np.inf, dtype=np.float32)
    vec_closest_x = np.zeros_like(u, dtype=np.float32)
    vec_closest_y = np.zeros_like(v, dtype=np.float32)
    ...
    for oy in range(-1, 2):
        for ox in range(-1, 2):
            ...
            vec_closest_x = np.where(mask, vec_x, vec_closest_x)
            vec_closest_y = np.where(mask, vec_y, vec_closest_y)

    min_edge = np.full(u.shape, np.inf, dtype=np.float32)
    for oy in range(-1, 2):
        for ox in range(-1, 2):
            cx = cell_x + ox + seed
            cy = cell_y + oy + seed
            ...
            inv_len = np.where(valid, 1.0 / np.sqrt(perp_len), 0.0)
            n_x = perp_x * inv_len
            n_y = perp_y * inv_len
            mid_x = (vec_closest_x + vec_x) * 0.5
            mid_y = (vec_closest_y + vec_y) * 0.5
            dist_edge = mid_x * n_x + mid_y * n_y
            dist_edge = np.where(valid, dist_edge, min_edge)
            min_edge = np.minimum(min_edge, dist_edge)
...
def _voronoi_n_sphere_radius(u, v, randomness, seed):
    ...
    min_dist = np.full(u.shape, np.inf, dtype=np.float32)
    closest_x = np.zeros_like(u, dtype=np.float32)
    closest_y = np.zeros_like(v, dtype=np.float32)
    closest_offset_x = np.zeros_like(cell_x)
    closest_offset_y = np.zeros_like(cell_y)
    ...
    for oy in range(-1, 2):
        for ox in range(-1, 2):
            ...
            mask = dist < min_dist
            min_dist = np.where(mask, dist, min_dist)
            closest_x = np.where(mask, point_x, closest_x)
            closest_y = np.where(mask, point_y, closest_y)
            closest_offset_x = np.where(mask, ox, closest_offset_x)
            closest_offset_y = np.where(mask, oy, closest_offset_y)

    min_dist = np.full(u.shape, np.inf, dtype=np.float32)
    closest2_x = np.zeros_like(u, dtype=np.float32)
    closest2_y = np.zeros_like(v, dtype=np.float32)
    for oy in range(-1, 2):
        for ox in range(-1, 2):
            skip = (ox == 0) & (oy == 0)
            ...
            dist = dx * dx + dy * dy
            dist = np.where(skip, min_dist, dist)
            mask = dist < min_dist
            min_dist = np.where(mask, dist, min_dist)
            closest2_x = np.where(mask, point_x, closest2_x)
            closest2_y = np.where(mask, point_y, closest2_y)

    return 0.5 * np.sqrt((closest2_x - closest_x) ** 2 + (closest2_y - closest_y) ** 2)
...
def _fractal_voronoi_distance(u, v, feat, randomness, metric, exponent, smoothness,
    ...
    max_amplitude = 0.0
    scale = 1.0
    output = np.zeros_like(u, dtype=np.float32)
    zero_input = detail == 0.0 or roughness == 0.0
    remainder = detail - np.floor(detail)

    for i in range(int(np.ceil(detail)) + 1):
...
def _fractal_voronoi_distance_to_edge(u, v, randomness, detail, roughness, lacunarity,
    ...
    scale = 1.0
    distance = np.full(u.shape, 8.0, dtype=np.float32)
    zero_input = detail == 0.0 or roughness == 0.0
    remainder = detail - np.floor(detail)
    ...
    for i in range(int(np.ceil(detail)) + 1):
        ...
        if i <= detail:
            ...
            distance = _lerp(distance, np.minimum(distance, octave / scale), amplitude)
            scale *= lacunarity
            amplitude *= roughness
        else:
            if remainder != 0.0:
                lerp_amp = _lerp(max_amplitude, max_distance / scale, amplitude)
                distance = _lerp(distance, np.minimum(distance, octave / scale), remainder)
...
def _fractal_voronoi_output(u, v, feat, randomness, metric, exponent, smoothness,
    ...
    max_amplitude = 0.0
    scale = 1.0
    out_distance = np.zeros_like(u, dtype=np.float32)
    out_r = np.zeros_like(u, dtype=np.float32)
    out_g = np.zeros_like(u, dtype=np.float32)
    out_b = np.zeros_like(u, dtype=np.float32)
    out_pos_x = np.zeros_like(u, dtype=np.float32)
    out_pos_y = np.zeros_like(v, dtype=np.float32)
    zero_input = detail == 0.0 or roughness == 0.0
    remainder = detail - np.floor(detail)
    ...
    if node_scale != 0.0:
        ...
        out_pos_x = np.zeros_like(out_pos_x)
        out_pos_y = np.zeros_like(out_pos_y)

    return out_distance, (out_r, out_g, out_b), (out_pos_x, out_pos_y)


def voronoi_texture(coords, seed=0, randomness=1.0,
    ...
    smooth = float(np.clip(smoothness / 2.0, 0.0, 0.5))
    detail = float(np.clip(detail, 0.0, 15.0))
    ...
    if out == "color":
        return np.stack([cr, cg, cb], axis=-1).astype(np.float32)
    if out in ["position", "pos"]:
        return np.stack([px, py, np.zeros_like(px)], axis=-1).astype(np.float32)
    if out == "radius":
        return _voronoi_n_sphere_radius(u, v, randomness, seed)
