"""
Noise Master - Perlin/Fractal Noise (CPU).
"""
...
from .noise_math import lerp as _lerp
...
def _negate_if(val, mask):
    return np.where(mask != 0, -val, val)


def _grad2(h, x, y):
    h = h & 7
    u = np.where(h < 4, x, y)
    v = np.where(h < 4, y, x) * 2.0
...
def _wrap_coord(p):
    correction = 0.5 * (np.abs(p) >= 1000000.0)
    return np.fmod(p, 100000.0) + correction
...
def _perlin_2d(x, y, seed=0):
    ...
    xi_u = xi.astype(np.uint32)
    yi_u = yi.astype(np.uint32)
    s = np.uint32(seed)
...
def _perlin_5d(x, y, z, w, v_dim, seed=0):
    ...
    zi = np.floor(z).astype(np.int32)
    wi = np.floor(w).astype(np.int32)
    vi = np.floor(v_dim).astype(np.int32)
...
def _random_float2_offset(seed):
    sx = hash_float2_to_float(np.float32(seed), np.float32(0.0))
    sy = hash_float2_to_float(np.float32(seed), np.float32(1.0))
    return (100.0 + float(sx) * 100.0, 100.0 + float(sy) * 100.0)


def _random_float3_offset(seed):
    s0 = hash_float2_to_float(np.float32(seed), np.float32(0.0))
    s1 = hash_float2_to_float(np.float32(seed), np.float32(1.0))
    s2 = hash_float2_to_float(np.float32(seed), np.float32(2.0))
...
def _random_float4_offset(seed):
    ...
    s1 = hash_float2_to_float(np.float32(seed), np.float32(1.0))
    s2 = hash_float2_to_float(np.float32(seed), np.float32(2.0))
    s3 = hash_float2_to_float(np.float32(seed), np.float32(3.0))
    return (100.0 + float(s0) * 100.0,
            100.0 + float(s1) * 100.0,
            100.0 + float(s2) * 100.0,
...
def _random_float5_offset(seed):
    ...
    s2 = hash_float2_to_float(np.float32(seed), np.float32(2.0))
    s3 = hash_float2_to_float(np.float32(seed), np.float32(3.0))
    s4 = hash_float2_to_float(np.float32(seed), np.float32(4.0))
    return (100.0 + float(s0) * 100.0,
            100.0 + float(s1) * 100.0,
            100.0 + float(s2) * 100.0,
            100.0 + float(s3) * 100.0,
...
def _perlin_distortion(coords, strength, seed=0):
    if strength == 0.0:
        return tuple(np.zeros_like(coords[0], dtype=np.float32) for _ in coords)
    dims = len(coords)
    if dims == 2:
...
def _perlin_multi_fractal(coords, detail, roughness, lacunarity, seed=0, cancel_check=None):
    value = np.ones_like(coords[0], dtype=np.float32)
    pwr = 1.0
    coords_local = coords
...
def _perlin_hybrid_multi_fractal(coords, detail, roughness, lacunarity, offset, gain, seed=0, cancel_check=None):
    pwr = 1.0
    value = np.zeros_like(coords[0], dtype=np.float32)
    weight = np.ones_like(coords[0], dtype=np.float32)
    coords_local = coords
    ...
    for _ in range(int(detail) + 1):
        ...
        value = np.where(active, value + weight * signal, value)
        weight = np.where(active, weight * gain * signal, weight)
        pwr *= roughness
        coords_local = tuple(c * lacunarity for c in coords_local)
...
def _perlin_ridged_multi_fractal(coords, detail, roughness, lacunarity, offset, gain, seed=0, cancel_check=None):
    ...
    for _ in range(1, int(detail) + 1):
        ...
        signal = offset - np.abs(_perlin_signed(coords_local, seed=seed))
        signal = signal * signal
...
def _perlin_fbm(coords, detail, roughness, lacunarity, normalize, seed=0, cancel_check=None):
    ...
    amp = 1.0
    maxamp = 0.0
    total = np.zeros_like(coords[0], dtype=np.float32)
    ...
    if rmd != 0.0:
        ...
        if normalize:
            return _lerp(0.5 * total / maxamp + 0.5,
                         ...
                         rmd).astype(np.float32)
        return _lerp(total, sum2, rmd).astype(np.float32)
    return (0.5 * total / maxamp + 0.5).astype(np.float32) if normalize else total.astype(np.float32)
...
def perlin_fractal_distorted(coords, detail, roughness, lacunarity,
