...
def _load_exr(path: Path, channel: str) -> Optional[np.ndarray]:
...
def compute_diff(img_a: np.ndarray, img_b: np.ndarray) -> Tuple[np.ndarray, int]:
    """
...
def create_side_by_side(img_a: np.ndarray, img_b: np.ndarray, 
                        diff: Optional[np.ndarray] = None) -> Image.Image:
