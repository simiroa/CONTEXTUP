...
class FinderApp(ctk.CTk):
    ...
    def _setup_ui(self):
        ...
        # 1. Scope
        ctk.CTkLabel(self.frm_sidebar, text="Scan Scope", font=ctk.CTkFont(size=14, weight="bold")).pack(pady=(15, 5), padx=8, anchor="w")
        ...
        self.lbl_path = ctk.CTkLabel(self.frm_sidebar, text=self.target_path or "No path selected", text_color="gray", wraplength=130)
        ...
        ctk.CTkCheckBox(tab_simple, text="Dup Hash", variable=self.var_chk_hash).pack(anchor="w", pady=2, padx=5)
        
        # Smart Tab Content - Version/Sequence detection
        ctk.CTkLabel(tab_smart, text="🔍 Smart Analysis", font=ctk.CTkFont(size=12, weight="bold")).pack(anchor="w", pady=(5, 2), padx=5)
        ctk.CTkLabel(tab_smart, text="Detects versions", text_color="gray", font=("Arial", 11)).pack(anchor="w", padx=5)
        ctk.CTkLabel(tab_smart, text="and sequences", text_color="gray", font=("Arial", 11)).pack(anchor="w", padx=5)
        ctk.CTkLabel(tab_smart, text="📁 SEQ/VER", text_color="#3B8ED0", font=("Arial", 10)).pack(anchor="w", pady=(5, 0), padx=5)
        ...
        # 3. Selection Tools
        ctk.CTkLabel(self.frm_sidebar, text="Select Tools", font=ctk.CTkFont(size=14, weight="bold")).pack(pady=(15, 8), padx=8, anchor="w")
        
        # --- Group 1: Pattern Search ---
        self.entry_pattern = ctk.CTkEntry(self.frm_sidebar, placeholder_text="Pattern...", height=32)
        ...
        ctk.CTkButton(self.frm_pattern, text="Sel", height=28, width=40, command=lambda: self._action_pattern(False)).pack(side="left", padx=(0, 3), expand=True, fill="x")
        ctk.CTkButton(self.frm_pattern, text="Keep", height=28, width=40, fg_color="#2B7A3D", hover_color="#236B32", command=lambda: self._action_pattern(True)).pack(side="left", padx=(0, 3), expand=True, fill="x")
        ctk.CTkButton(self.frm_pattern, text="Inv", height=28, width=40, fg_color="gray50", command=self._invert_selection).pack(side="left", expand=True, fill="x")
        ...
        ctk.CTkButton(self.frm_date, text="Oldest", height=28, fg_color="#1A5C8A", hover_color="#14496E", command=self._select_keep_oldest).pack(side="left", padx=(0, 3), expand=True, fill="x")
        ctk.CTkButton(self.frm_date, text="Newest", height=28, fg_color="#1A5C8A", hover_color="#14496E", command=self._select_keep_newest).pack(side="left", expand=True, fill="x")
        ...
        ctk.CTkButton(self.frm_bulk, text="All", height=28, fg_color="gray45", command=lambda: self._select_all(True)).pack(side="left", padx=(0, 3), expand=True, fill="x")
        ctk.CTkButton(self.frm_bulk, text="None", height=28, fg_color="gray45", command=lambda: self._select_all(False)).pack(side="left", expand=True, fill="x")

        # 4. Action
        self.lbl_stats = ctk.CTkLabel(self.frm_sidebar, text="Ready")
        self.lbl_stats.pack(side="bottom", pady=10)
        
        self.btn_delete = ctk.CTkButton(self.frm_sidebar, text="DELETE", fg_color="red", state="disabled", command=self._delete_selected)
    ...
    def _update_global_stats(self):
        ...
        self.lbl_stats.configure(text=f"{len(self.all_groups)} groups | {total_selected}/{total_files} selected")
    ...
    def _delete_selected(self):
        ...
        if send2trash is None:
            if not messagebox.askyesno(
                "⚠️ 주의: 영구 삭제",
                f"send2trash가 설치되지 않아 {len(to_delete)}개 파일이 **영구 삭제**됩니다.\n"
                "휴지통으로 이동하지 않습니다!\n\n정말 삭제하시겠습니까?",
                icon="warning"
            ):
                return
