...
class CreativeStudioGUI(PremiumComfyWindow):
    def __init__(self):
        super().__init__(title="ComfyUI Creative Studio (Z) [Pro]", width=1360, height=900)
    ...
    def _setup_layout(self):
        ...
        self.preview_label = ctk.CTkLabel(self.viewport, text="Z-Engine Prime Ready.", font=Fonts.HEADER, text_color=Colors.TEXT_TERTIARY)
    ...
    def _build_config_section(self, parent):
        PremiumLabel(parent, text="GENERATION CONFIG", style="header").pack(anchor="w", pady=(0, 10))
        ...
        # Sliders
        self.widgets["steps"] = ValueSliderWidget(parent, "Steps", {"from": 1, "to": 50, "res": 1, "default": 4})
        self.widgets["cfg"] = ValueSliderWidget(parent, "Magic (CFG)", {"from": 1, "to": 20, "res": 0.1, "default": 2.0})
        self.widgets["batch_size"] = ValueSliderWidget(parent, "Batch Size", {"from": 1, "to": 8, "res": 1, "default": 1})
    ...
    def _on_ai_refine(self, layer_widget):
        ...
        self.status_badge.set_status("AI Polishing...", "active")
        ...
        def _cb(res, err=None):
            ...
            if res:
                ...
                self.after(0, lambda: self.status_badge.set_status("AI Enhanced", "success") )
            else:
    ...
    def _build_ignite_section(self, parent):
        ...
        self.widgets["upscale"] = ctk.CTkCheckBox(opts, text="Upscale (2x)", font=Fonts.BODY, fg_color=Colors.ACCENT_PRIMARY)
        self.widgets["upscale"].pack(side="left", padx=5)
        
        self.widgets["rembg"] = ctk.CTkCheckBox(opts, text="Remove BG", font=Fonts.BODY, fg_color=Colors.ACCENT_PURPLE)
        ...
        self.btn_ignite = ActionButton(action_row, text="Ignite (Z)", variant="magic", height=55, command=self.start_generation)
    ...
    def start_generation(self):
        ...
        self.btn_ignite.configure(state="disabled", text="Synthesizing...")
        self.status_badge.set_status("Igniting Z-Turbo", "active")
    ...
    def _on_success(self, images):
        self.is_processing = False
        self.btn_ignite.configure(state="normal", text="Ignite (Z)")
        self.status_badge.set_status("Success", "success")
        if images:
