...
def bake(high_path, low_path, out_path, size=1024, margin=16, ray_dist=0.1, 
    ...
    if not bpy.context.selected_objects:
        print(f"Failed to import low poly: {low_path}")
        return
    ...
    # Try GPU, fallback to CPU if needed (Blender handles this usually, but CYCLES_DEVICE can be set)
    ...
    if bake_orm and img_r and img_g and img_b:
        ...
        tree.links.new(in_r.outputs[0], comb.inputs[0])
        tree.links.new(in_g.outputs[0], comb.inputs[1])
        tree.links.new(in_b.outputs[0], comb.inputs[2])
...
if __name__ == "__main__":
    ...
    parser.add_argument("--bake_normal", action="store_true")
