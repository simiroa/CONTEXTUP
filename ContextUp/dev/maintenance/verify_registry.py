import winreg
import sys

def check_registry():
    print("Checking Registry for 'tool_translator'...")
    
    roots = [
        ("File (*)", r"Software\Classes\*\shell\ContextUp"),
        ("Folder (Directory)", r"Software\Classes\Directory\shell\ContextUp"),
        ("Background", r"Software\Classes\Directory\Background\shell\ContextUp")
    ]
    
    found_any = False
    
    for label, root_path in roots:
        print(f"\n--- Scanning {label} ---")
        try:
            # Check Parent First
            with winreg.OpenKey(winreg.HKEY_CURRENT_USER, root_path) as key:
                print(f"  [OK] Submenu Key exists: {root_path}")
                try:
                    val, _ = winreg.QueryValueEx(key, "AppliesTo")
                    print(f"  [INFO] Submenu AppliesTo: {val}")
                except:
                    print(f"  [INFO] Submenu AppliesTo: (None)")
                
                # Check Shell Subkey
                shell_path = f"{root_path}\\shell"
                with winreg.OpenKey(winreg.HKEY_CURRENT_USER, shell_path) as shell_key:
                     i = 0
                     found_in_root = False
                     while True:
                        try:
                            subkey_cmd = winreg.EnumKey(shell_key, i)
                            if "tool_translator" in subkey_cmd:
                                print(f"  [FOUND] Item: {subkey_cmd}")
                                found_in_root = True
                                found_any = True
                                
                                # Check Item AppliesTo
                                item_path = f"{shell_path}\\{subkey_cmd}"
                                with winreg.OpenKey(winreg.HKEY_CURRENT_USER, item_path) as k:
                                    try:
                                        val, _ = winreg.QueryValueEx(k, "AppliesTo")
                                        print(f"    Item AppliesTo: {val}")
                                    except:
                                        print(f"    Item AppliesTo: (None)")
                            i += 1
                        except OSError:
                            break
                     
                     if not found_in_root:
                         print("  [MISSING] Item not found in this scope.")

        except FileNotFoundError:
            print(f"  [MISSING] Submenu Key not found: {root_path}")
        except Exception as e:
            print(f"  [ERROR] {e}")

    if found_any:
        print("\nResult: Found in at least one scope.")
    else:
        print("\nResult: NOT FOUND anywhere.")

if __name__ == "__main__":
    check_registry()
