...
class ConvertMeshGUI(BaseWindow):
    def __init__(self, target_path):
        super().__init__(title="ContextUp Mesh Converter", width=600, height=500)
...
class OptimizeMeshGUI(BaseWindow):
    def __init__(self, target_path):
        super().__init__(title="ContextUp Mesh Optimizer", width=500, height=400)
...
def extract_textures(target_path: str):
    ...
    try:
        ...
        # Use a simple window just to show progress/blocking? Or just run it?
        # Since it's a single file and usually fast, we can just run it, but showing a "Processing" dialog is better.
        # For now, let's keep it simple as it wasn't explicitly requested to be a full GUI, 
        # but we should avoid the double window issue.
